...
function analiseFaturamento(dados: { dia: number, valor: number }[]): { menor: number, maior: number, diasAcimaMedia: number } {
    const valores = dados.filter(d => d.valor > 0).map(d => d.valor);
    const mediaMensal = valores.reduce((sum, val) => sum + val, 0) / valores.length;
